...
function getCSVchoice() {
  ...
  Dialog.addHelp("<html>Use this Menu Tool to spatially calibrate images lacking detailed metadata.<p><p>"
        +"<b>Select the csv file corresponding to the device used to acquire the image</b>.<p>"
        +"Select <i>Demo File...</i> to generate a template to be edited by an external spreadsheet<p>"
        +"application.<p><p>"
        +"<b>To apply global calibrations</b>, i.e., to propagate the chosen calibration to all open<p>"
        +"images, <b>activate the <i>Global</i> checkbox in Image>Properties... [P]</b> (titles of image<p>"
        +"windows will be labeled with a '<tt>(G)</tt>').<p><p>"
        +"Calibration settings are loaded from <tt>.csv</tt> files stored in:<p>"
